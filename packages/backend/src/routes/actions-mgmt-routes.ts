...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    ...
    const { data, error } = await Postgres
      ...
      .eq('id', id)
      .single();
    if (error || !data) return res.status(404).json({ error: 'Acción no encontrada' });
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const { data: existing } = await Postgres
      ...
      .maybeSingle();
    if (existing) return res.status(409).json({ error: 'Ya existe una acción con esa clave' });
    ...
    if (action_key) {
      ...
      if (action_key.toUpperCase() !== existing.action_key) {
        const { data: dup } = await Postgres
          ...
          .neq('id', id)
          .maybeSingle();
        if (dup) return res.status(409).json({ error: 'Ya existe una acción con esa clave' });
      }
    }
