...
import { ensureSystemAdmin, getWizardState, getBootstrapToken, getSystemLanguages, bootstrapStep1Tenant, bootstrapStep2Admin, } from './bootstrap';
// Importar funciones de tenant
import { getTenant, updateTenant, getTenantSettings, createTenantSetting, updateTenantSetting, deleteTenantSetting, getTenantMembers, getDataTypes, getTenantLanguages, updateTenantLanguages, ensureMainTenant, getSystemTenantSettings, updateSystemTenantSettings, } from './tenant-routes';
...
router.get('/bootstrap/ping', (req, res) => {
...
router.get('/bootstrap/tenant-info', requireAuth, async (req, res) => {
    try {
...
router.get('/users/menu-screens', requireAuth, async (req, res) => {
    try {
        const user = req.user;
...
router.post('/auth/create-system-admin', async (req, res) => {
    try {
        const Postgres = getPostgresClient();
        const body = req.body || {};
...
router.post('/auth/reset-system-admin-password', async (req, res) => {
    try {
        const Postgres = getPostgresClient();
...
router.post('/db/query', requireAuth, async (req, res) => {
    try {
        const Postgres = getPostgresClient();
        const { table, action, select, filters, order, limit, payload, upsertOptions, single, maybeSingle, } = req.body || {};
        if (!table || typeof table !== 'string') {
            return res.status(400).json({ error: 'table es requerido' });
        }
        let query = Postgres.from(table);
        if (action === 'insert') {
