...
router.post('/', async (req, res) => {
    try {
        const body = req.body;
        const { lookup_group_key, lookup_group_label, lookup_group_short_label, allows_tenant_items, is_active, translations } = body;
        // Validaciones
...
router.put('/:id', async (req, res) => {
    try {
        const id = req.params.id;
        const body = req.body;
        const { lookup_group_label, lookup_group_short_label, allows_tenant_items, is_active, translations } = body;
        // Validaciones
        if (!lookup_group_label?.trim()) {
            return res.status(400).json({ error: 'La etiqueta del grupo es obligatoria' });
        }
...
//# sourceMappingURL=lookup-groups-routes.js.map