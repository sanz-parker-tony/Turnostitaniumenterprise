...
router.post('/', async (req, res) => {
    try {
        const body = req.body;
        const { tenant_id, role_key, role_name, role_scope = 'TENANT', base_role_id, data_scope = 'ALL', is_active = true, } = body;
...
router.put('/:id', async (req, res) => {
    try {
        ...
        if (role_key !== undefined)
            updateData.role_key = role_key.toUpperCase();
        if (role_name !== undefined)
            updateData.role_name = role_name;
        if (role_scope !== undefined)
            updateData.role_scope = role_scope;
        if (base_role_id !== undefined)
            updateData.base_role_id = base_role_id || null;
