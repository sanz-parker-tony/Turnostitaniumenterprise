...
router.get('/catalogs/screen-actions', async (req, res) => {
    try {
        const Postgres = getPostgres();
        const { data, error } = await Postgres
            .from('screen_actions')
...
router.get('/', async (req, res) => {
    try {
        ...
        const screenKey = req.query.screen_key;
        let query = Postgres
      ...
      `)
        ...
        const result = (data || []).map((rsa) => ({
            ...
            if (screenKey)
                return rsa.screen_key === screenKey;
            return true;
        });
...
router.post('/', async (req, res) => {
    try {
        ...
        const { data: existing } = await Postgres
            ...
            .maybeSingle();
        if (existing)
            return res.status(409).json({ error: 'Ya existe ese permiso para ese rol y pantalla-acción' });
        const { data, error } = await Postgres
            .from('role_screen_actions')
            .insert({
            tenant_id,
...
router.put('/:id', async (req, res) => {
    try {
        ...
        if (is_allowed !== undefined)
            updateData.is_allowed = is_allowed;
...
// ── PATCH /:id/allowed ───────────────────────────────────────────────────────
router.patch('/:id/allowed', async (req, res) => {
    try {
        const id = req.params.id;
...
//# sourceMappingURL=role-screen-actions-mgmt-routes.js.map