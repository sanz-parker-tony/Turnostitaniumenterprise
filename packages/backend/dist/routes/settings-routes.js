...
function validateSettingValue(value, typeKey) {
    if (!typeKey || !value)
        return null;
...
async function getTypeKey(Postgres, valueTypeId) {
    if (!valueTypeId)
        return null;
...
router.post('/system-settings', async (req, res) => {
    try {
        ...
        if (!setting_key?.trim())
            return res.status(400).json({ error: "setting_key es obligatorio" });
        if (!setting_name?.trim())
            return res.status(400).json({ error: "setting_name es obligatorio" });
        if (!setting_short_key?.trim())
            return res.status(400).json({ error: "setting_short_key es obligatorio" });
...
router.get('/effective', async (req, res) => {
    try {
        ...
        if (!result)
            return res.status(404).json({ error: `Parámetro '${settingKey}' no encontrado` });
...
router.get('/all-effective', async (req, res) => {
    try {
        const tenantId = req.query.tenant_id;
        const companyId = req.query.company_id;
        const profileId = req.query.profile_id;
        ...
        const tenantMap = new Map((tenantOverrides.data ?? []).map((r) => [r.system_setting_id, toNullableString(r.setting_value)]));
        const companyMap = new Map((companyOverrides.data ?? []).map((r) => [r.system_setting_id, toNullableString(r.setting_value)]));
        const profileMap = new Map((profileOverrides.data ?? []).map((r) => [r.system_setting_id, toNullableString(r.setting_value)]));
        const results = allSettings.map((ss) => {
            let effectiveValue = ss.default_value;
            let localValue = null;
            let sourceLevel = "SYSTEM";
...
async function resolveEffectiveSetting(Postgres, opts) {
...
// ============================================================================
router.get('/tenants/:id/settings-overrides', async (req, res) => {
    try {
...
router.post('/tenants/:id/settings-overrides', async (req, res) => {
    try {
        const tenantId = req.params.id;
        const body = req.body;
        const { system_setting_id, setting_value, created_by, is_active } = body;
        if (!system_setting_id)
            return res.status(400).json({ error: "system_setting_id es obligatorio" });
...
router.delete('/tenants/:id/settings-overrides/:setting_id', async (req, res) => {
    try {
        const tenantId = req.params.id;
...
// ============================================================================
router.get('/companies/:id/settings-overrides', async (req, res) => {
    try {
...
router.post('/companies/:id/settings-overrides', async (req, res) => {
    try {
        const companyId = req.params.id;
        const body = req.body;
...
router.delete('/companies/:id/settings-overrides/:setting_id', async (req, res) => {
    try {
        const companyId = req.params.id;
        const settingId = req.params.setting_id;
        const Postgres = getPostgresClient();
...
// ============================================================================
router.get('/employee-profiles/:id/settings-overrides', async (req, res) => {
    try {
...
router.post('/employee-profiles/:id/settings-overrides', async (req, res) => {
    try {
        const profileId = req.params.id;
        const body = req.body;
...
router.delete('/employee-profiles/:id/settings-overrides/:setting_id', async (req, res) => {
    try {
        const profileId = req.params.id;
        const settingId = req.params.setting_id;
        const tenantId = req.query.tenant_id;
...
// ============================================================================
router.get('/lookup-values/setting-data-types', async (req, res) => {
    try {
        const Postgres = getPostgresClient();
...
router.get('/verify', async (req, res) => {
    try {
        ...
        if (e1)
            results.test1_error = e1.message;
        ...
        if (e3)
            results.test3_error = e3.message;
        if (allSettings && allSettings.length > 0) {
