...
router.get('/catalogs/permission-levels', (req, res) => {
...
router.get('/:id', async (req, res) => {
    try {
        ...
        const { data, error } = await Postgres
            ...
            .single();
        if (error || !data)
            return res.status(404).json({ error: 'Grupo de menú no encontrado' });
...
router.post('/', async (req, res) => {
    try {
        ...
        const { data: existing } = await Postgres
            ...
            .maybeSingle();
        if (existing)
            return res.status(409).json({ error: 'Ya existe un grupo con esa clave' });
        const { data, error } = await Postgres
            .from('system_menu_groups')
            .insert({
...
router.put('/:id', async (req, res) => {
    try {
        ...
        if (menu_group_key !== undefined)
            updateData.menu_group_key = menu_group_key.toUpperCase();
        if (menu_group_name !== undefined)
            updateData.menu_group_name = menu_group_name;
        if (menu_group_short_name !== undefined)
            updateData.menu_group_short_name = menu_group_short_name || null;
        if (icon_key !== undefined)
            updateData.icon_key = icon_key || null;
        if (sort_order !== undefined)
            updateData.sort_order = sort_order;
        if (permission_level !== undefined)
            updateData.permission_level = permission_level || null;
...
//# sourceMappingURL=menu-groups-routes.js.map