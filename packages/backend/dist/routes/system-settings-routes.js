...
router.post('/', async (req, res) => {
    try {
        const body = req.body;
        const { setting_key, setting_name, setting_short_key, value_type_id, default_value, description, allowed_lookup_group_id, is_active = true, } = body;
        // Validaciones
...
router.put('/:id', async (req, res) => {
    try {
        const id = req.params.id;
        const body = req.body;
        const { setting_key, setting_name, setting_short_key, value_type_id, default_value, description, allowed_lookup_group_id, is_active, } = body;
        ...
        if (setting_key !== undefined)
            updateData.setting_key = setting_key.toUpperCase();
        if (setting_name !== undefined)
            updateData.setting_name = setting_name;
        if (setting_short_key !== undefined)
            updateData.setting_short_key = setting_short_key.toUpperCase();
        if (value_type_id !== undefined)
            updateData.value_type_id = value_type_id;
        if (default_value !== undefined)
            updateData.default_value = default_value || null;
        if (description !== undefined)
            updateData.description = description || null;
        if (allowed_lookup_group_id !== undefined)
            updateData.allowed_lookup_group_id = allowed_lookup_group_id || null;
