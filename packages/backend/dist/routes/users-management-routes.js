...
router.get('/catalogs/tenants', async (req, res) => {
    try {
...
router.get('/catalogs/roles', async (req, res) => {
    try {
        const Postgres = getPostgres();
        const { data, error } = await Postgres
            .from('roles')
...
router.get('/catalogs/scope-types', async (req, res) => {
    try {
        const Postgres = getPostgres();
        const { data, error } = await Postgres
            .from('scope_types')
...
router.get('/catalogs/companies', async (req, res) => {
    try {
        const Postgres = getPostgres();
        const { data, error } = await Postgres
            .from('companies')
...
router.put('/user-roles/:user_role_id', async (req, res) => {
    try {
        ...
        if (valid_from !== undefined)
            updateData.valid_from = valid_from || null;
        if (valid_to !== undefined)
            updateData.valid_to = valid_to || null;
...
router.patch('/user-roles/:user_role_id/status', async (req, res) => {
    try {
        const userRoleId = req.params.user_role_id;
        const body = req.body;
...
router.get('/user-roles/:user_role_id/scopes', async (req, res) => {
    try {
        const userRoleId = req.params.user_role_id;
        const Postgres = getPostgres();
...
router.post('/user-roles/:user_role_id/scopes', async (req, res) => {
    try {
        const userRoleId = req.params.user_role_id;
        const body = req.body;
...
router.patch('/scopes/:scope_id/status', async (req, res) => {
    try {
...
router.post('/', async (req, res) => {
    try {
        const body = req.body;
        const { tenant_id, username, display_name, email, phone, preferred_language_code, password, is_active = true, } = body;
...
router.put('/:id', async (req, res) => {
    try {
        ...
        if (preferred_language_code !== undefined)
            updateData.preferred_language_code = preferred_language_code || null;
        if (is_active !== undefined)
            updateData.is_active = is_active;
...
router.get('/:user_id/roles', async (req, res) => {
    try {
...
router.post('/:user_id/roles', async (req, res) => {
    try {
        const userId = req.params.user_id;
        const body = req.body;
