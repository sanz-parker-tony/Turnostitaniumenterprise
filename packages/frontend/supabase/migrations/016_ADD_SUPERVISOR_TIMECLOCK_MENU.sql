-- ============================================================================
-- 016_ADD_SUPERVISOR_TIMECLOCK_MENU
-- Crea/asegura tabla employee_time_punches y habilita menú Marcaciones
-- para rol SUPERVISOR en grupo Asistencia
-- ============================================================================
...
(
    ...
    CONSTRAINT employee_time_punches_company_id_fkey FOREIGN KEY (company_id)
        REFERENCES public.companies (id) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION,
    CONSTRAINT employee_time_punches_employee_id_fkey FOREIGN KEY (employee_id)
        REFERENCES public.employees (id) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION,
    CONSTRAINT employee_time_punches_punch_source_id_fkey FOREIGN KEY (punch_source_id)
        REFERENCES public.lookup_values (id) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION,
    CONSTRAINT employee_time_punches_tenant_id_fkey FOREIGN KEY (tenant_id)
        REFERENCES public.tenants (id) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION,
    CONSTRAINT employee_time_punches_time_clock_device_id_fkey FOREIGN KEY (time_clock_device_id)
        REFERENCES public.time_clock_devices (id) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION,
    CONSTRAINT employee_time_punches_time_punch_status_id_fkey FOREIGN KEY (time_punch_status_id)
        REFERENCES public.lookup_values (id) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION,
    CONSTRAINT fk_employee_time_punches_run FOREIGN KEY (process_run_id)
        REFERENCES public.attendance_processing_runs (id) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION
);

CREATE INDEX IF NOT EXISTS idx_employee_time_punches_tenant_datetime
  ON public.employee_time_punches (tenant_id, punch_datetime DESC, is_active);

CREATE INDEX IF NOT EXISTS idx_employee_time_punches_employee_datetime
  ON public.employee_time_punches (employee_id, punch_datetime DESC);
...
BEGIN
  ...
  IF v_attendance_group_id IS NULL THEN
    RAISE NOTICE 'No existe menu_group ATTENDANCE. Se omite alta de menú Marcaciones.';
    RETURN;
  END IF;

  SELECT id INTO v_screen_id
  FROM public.screens
  WHERE screen_key = 'TIMECLOCK_MANAGEMENT'
  LIMIT 1;
  ...
  IF v_screen_id IS NULL THEN
    INSERT INTO public.screens (
      ...
      '/dashboard/attendance/timeclock',
      'Clock',
      106,
      true,
      'SYSTEM'
  ...
  ELSE
    UPDATE public.screens
       SET screen_name = 'Gestión de Marcaciones',
           menu_label = 'Marcaciones',
           menu_group_id = v_attendance_group_id,
           route_path = '/dashboard/attendance/timeclock',
           icon_key = 'Clock',
           sort_order = 106,
           is_active = true,
           updated_by = 'SYSTEM',
