...
export default function WizardStepTenant({ onComplete, onGoBack }: WizardStepTenantProps) {
  ...
  const loadBootstrapToken = async () => {
    ...
    try {
      ...
      const urlDirect = `https://${projectId}.supabase.co/functions/v1/make-server-e19f2094/bootstrap/token-direct`;
      console.log('🔗 Intentando endpoint directo:', urlDirect);
      
      // IMPORTANTE: Supabase Edge Functions requiere Authorization header
      // Usamos publicAnonKey aunque no lo validemos en bootstrap mode
      ...
      if (!response.ok) {
        ...
        const urlModule = `https://${projectId}.supabase.co/functions/v1/make-server-e19f2094/bootstrap/token`;
  ...
  const fetchLanguages = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-e19f2094/bootstrap/languages`,
        {
          headers: {
            'Authorization': `Bearer ${publicAnonKey}`,
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-e19f2094/bootstrap/step1-tenant`,
        {
          method: 'POST',
          headers: {
