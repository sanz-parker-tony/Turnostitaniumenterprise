...
export default function WizardStepStructure({ onComplete, onGoBack }: WizardStepStructureProps) {
  ...
  const handleDownloadTemplate = async (entityKey: EntityType) => {
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-e19f2094/bootstrap/tenant-info`,
        {
          headers: {
            'Authorization': `Bearer ${publicAnonKey}`,
  ...
  const handleEntityUpload = async (
    ...
    try {
      ...
      // Enviar al servidor
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-e19f2094/${endpoint}`,
        {
          method: 'POST',
          headers: {
  ...
  const handleWorkLocationsUpload = async (file: File) => {
    try {
      ...
      // Enviar al servidor
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-e19f2094/bootstrap/step3-structure/work-locations`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
