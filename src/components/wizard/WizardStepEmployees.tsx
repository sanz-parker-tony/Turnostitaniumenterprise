...
export default function WizardStepEmployees({ onComplete, onGoBack }: WizardStepEmployeesProps) {
  ...
  const loadCatalogsAndTenantInfo = async () => {
    try {
      ...
      if (!bootstrapToken) {
        ...
        try {
          const tokenResponse = await fetch(
            `https://${projectId}.supabase.co/functions/v1/make-server-e19f2094/bootstrap/token-direct`,
            {
              headers: {
      ...
      const [catalogsResponse, tenantInfoResponse] = await Promise.all([
        fetch(
          `https://${projectId}.supabase.co/functions/v1/make-server-e19f2094/bootstrap/catalogs`,
          {
            headers: {
              'Authorization': `Bearer ${publicAnonKey}`,
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-e19f2094/bootstrap/employees`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${publicAnonKey}`,
  ...
  const handleContinue = async () => {
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-e19f2094/bootstrap/update-step`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${publicAnonKey}`,
