'use client';
/**
 * ScreenActionsManagement - Acciones por Pantalla
 * Turnos Titanium Enterprise — Seguridad → Acciones de Pantalla
 */
...
const API = `https://${projectId}.supabase.co/functions/v1/make-server-e19f2094/screen-actions-management`;
function getToken() { return localStorage.getItem('sb-access-token') || publicAnonKey; }

interface ScreenAction {
  id: string; screen_id: string; action_id: string; ui_element_key: string | null; is_active: boolean;
  created_at: string; updated_at: string | null;
  screen_key?: string | null; screen_name?: string | null; screen_menu_label?: string | null;
  action_key?: string | null; action_name?: string | null;
}
interface ScreenCatalog { id: string; screen_key: string; screen_name: string; menu_label: string | null; }
...
export function ScreenActionsManagement() {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (!saRes.ok) throw new Error(saData.error || 'Error cargando datos');
  ...
  const filtered = useMemo(() => {
    let list = [...screenActions];
    if (search) {
      const s = search.toLowerCase();
      list = list.filter(sa => (sa.screen_name || '').toLowerCase().includes(s) || (sa.action_name || '').toLowerCase().includes(s) || (sa.ui_element_key || '').toLowerCase().includes(s));
    }
    ...
    return list;
  }, [screenActions, search, statusFilter, screenFilter]);

  const openCreate = () => { setEditing(null); setForm({ screen_id: '', action_id: '', ui_element_key: '', is_active: true }); setPanelError(null); setPanelOpen(true); };
  const openEdit = (sa: ScreenAction) => { setEditing(sa); setForm({ screen_id: sa.screen_id, action_id: sa.action_id, ui_element_key: sa.ui_element_key || '', is_active: sa.is_active }); setPanelError(null); setPanelOpen(true); };

  const save = async () => {
    setPanelError(null);
    if (!form.screen_id || !form.action_id) { setPanelError('Pantalla y acción son obligatorios'); return; }
    setSaving(true);
    try {
  ...
  const toggleStatus = async (sa: ScreenAction) => {
    try {
      await fetch(`${API}/${sa.id}/status`, { method: 'PATCH', headers: headers(), body: JSON.stringify({ is_active: !sa.is_active }) });
      await load();
    } catch (e: any) { setError(e.message); }
  };

  // Unique screens for filter
  const uniqueScreens = useMemo(() => {
    const seen = new Set<string>();
    return screenActions.filter(sa => { if (seen.has(sa.screen_id)) return false; seen.add(sa.screen_id); return true; });
  }, [screenActions]);
    ...
    <div className="flex flex-col h-full gap-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-lg bg-[#0074D9] flex items-center justify-center"><Link2 className="w-5 h-5 text-white" /></div>
          <div>
            <h1 className="text-xl font-bold text-gray-900">Acciones de Pantalla</h1>
            <p className="text-sm text-gray-500">Relación de acciones disponibles por pantalla del sistema</p>
          </div>
        </div>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="relative flex-1 min-w-48">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar por pantalla o acción..."
        ...
        <select value={screenFilter} onChange={e => setScreenFilter(e.target.value)}
          ...
          {uniqueScreens.map(sa => <option key={sa.screen_id} value={sa.screen_id}>{sa.screen_name}</option>)}
        </select>
                ...
                {filtered.length === 0 ? (
                  ...
                  <tr key={sa.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3">
                      <div>
                        <p className="font-medium text-gray-900">{sa.screen_name || '—'}</p>
                        <p className="text-xs text-gray-400 font-mono">{sa.screen_key}</p>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <div>
                        <p className="font-medium text-gray-900">{sa.action_name || '—'}</p>
                        <p className="text-xs text-gray-400 font-mono">{sa.action_key}</p>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-gray-500 font-mono text-xs">{sa.ui_element_key || '—'}</td>
                    <td className="px-4 py-3 text-center"><span className={sa.is_active ? BADGE.active : BADGE.inactive}>{sa.is_active ? 'Activo' : 'Inactivo'}</span></td>
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-center gap-1">
                        <button onClick={() => openEdit(sa)} className="p-1.5 text-gray-500 hover:text-blue-600 hover:bg-blue-50 rounded-lg"><Edit2 className="w-4 h-4" /></button>
                        <button onClick={() => toggleStatus(sa)} className={`p-1.5 rounded-lg ${sa.is_active ? 'text-gray-500 hover:text-orange-600 hover:bg-orange-50' : 'text-gray-500 hover:text-green-600 hover:bg-green-50'}`}>
            ...
            <div className="flex-1 overflow-y-auto p-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  {!!editing && <p className="text-xs text-gray-400 mt-1">La pantalla y acción no se pueden cambiar. Cree una nueva relación si necesita otro par.</p>}
                </div>
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-1">Identificador del elemento visual en la pantalla (opcional).</p>
                </div>
                <div className="flex items-center gap-3">
