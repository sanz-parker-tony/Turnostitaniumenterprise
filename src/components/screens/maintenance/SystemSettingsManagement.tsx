...
export function SystemSettingsManagement() {
  ...
  const loadSettings = async () => {
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-e19f2094/system-settings-management`,
        {
          headers: {
  ...
  const loadValueTypes = async () => {
    try {
      const token = localStorage.getItem('sb-access-token');
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-e19f2094/system-settings-management/catalogs/value-types`,
        {
          headers: {
            'Authorization': `Bearer ${token || publicAnonKey}`,
            'Content-Type': 'application/json',
  ...
  const loadLookupGroups = async () => {
    try {
      const token = localStorage.getItem('sb-access-token');
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-e19f2094/system-settings-management/catalogs/lookup-groups`,
        {
          headers: {
            'Authorization': `Bearer ${token || publicAnonKey}`,
            'Content-Type': 'application/json',
  ...
  const handleSave = async () => {
    ...
    try {
      const token = localStorage.getItem('sb-access-token');
      const url = editingId
        ? `https://${projectId}.supabase.co/functions/v1/make-server-e19f2094/system-settings-management/${editingId}`
        : `https://${projectId}.supabase.co/functions/v1/make-server-e19f2094/system-settings-management`;
  ...
  const handleToggleStatus = async (id: string, currentStatus: boolean) => {
    ...
    try {
      const token = localStorage.getItem('sb-access-token');
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-e19f2094/system-settings-management/${id}/status`,
        {
          method: 'PATCH',
          headers: {
            'Authorization': `Bearer ${token || publicAnonKey}`,
