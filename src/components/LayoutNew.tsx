...
import SystemMenuGroups from './SystemMenuGroups';
import SystemMenuGroupsAdmin from './admin/SystemMenuGroupsAdmin';
...
const SCREEN_COMPONENT_MAP: { [key: string]: { component: string; tab?: string } } = {
  ...
  'SEC_MENU_GROUPS': { component: 'system-menu-groups' },
  'SEC_SYSTEM_ADMIN': { component: 'system-administration' },
...
export default function Layout() {
  ...
  const renderContent = () => {
    ...
    const { screen, group } = currentGroup;
    const pageTitle = screen.screenName;
    
    // ✅ VALIDACIÓN DE PERMISOS - Super Admin tiene acceso total
    const isSuperAdmin = profile?.is_super_admin === true || profile?.role_scope === 'SYSTEM';
    const screenAllowed = menuScreens.some(s => s.screen_key === screen.screenKey);
    const hasPermission = isSuperAdmin || screenAllowed;
    
    console.log(`[LAYOUT] Validando acceso a ${screen.screenKey}:`, {
      isSuperAdmin,
      screenAllowed,
      hasPermission,
      profile: {
      ...
      return (
        <div className="flex items-center justify-center h-full">
          <div className="bg-red-50 border border-red-200 rounded-lg p-8 max-w-md">
            ...
            <h2 className="text-xl font-semibold text-red-900 mb-2 text-center">Acceso Denegado</h2>
    ...
    // ✅ RENDERIZAR COMPONENTE AUTORIZADO
    switch (screen.component) {
      ...
      case 'system-menu-groups':
        return <SystemMenuGroupsAdmin />;
      ...
      case 'kiosk-permission':
        return <KioskPermission />;
      case 'kiosk-justification':
        return <KioskJustification />;
      case 'kiosk-shift-change':
        return <KioskShiftChange />;
      default:
        return <Dashboard />;
    }
  };
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Top Bar - Fixed */}
      <div className="bg-white border-b border-border fixed top-0 left-0 right-0 z-40 shadow-sm">
      ...
      {/* Sidebar - Fixed */}
      <div
        className={`fixed left-0 top-16 bottom-0 w-72 bg-white border-r border-gray-200/80 transition-all duration-300 ${
