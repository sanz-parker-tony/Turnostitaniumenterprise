/* ================================================================================================
 Turnos Titanium — CORRECCIONES Y ADICIONES AL DDL BASE (CORREGIDO)
 Fecha: 2025-01-03
 Fix: Orden de parámetros en funciones (parámetros sin default ANTES de los con default)
================================================================================================ */

----------------------------------------------------------------------------------------------------
-- 1) AGREGAR CAMPO icon_key A SCREENS (si no se agregó en DDL base)
----------------------------------------------------------------------------------------------------

-- Este campo es CRÍTICO para el menú dinámico con iconMapper.tsx
-- NOTA: Si ya usaste 00_DDL_v2_CORREGIDO.sql, este campo YA EXISTE, este comando no hará nada
...
/* clone_role: Clona un rol completo creando uno nuevo
   Parámetros (ORDEN CORREGIDO: sin default primero, con default al final):
   ...
   - p_new_role_description: Descripción (opcional, DEFAULT NULL)
...
/* copy_role_permissions: Copia permisos entre roles existentes
   ...
   - p_copy_screen_actions: Copiar acciones de pantalla (DEFAULT true)
   - p_copy_reports: Copiar permisos de reportes (DEFAULT true)
   - p_copy_scopes: Copiar scopes (DEFAULT false)
...
create or replace function public.copy_role_permissions(
  ...
  p_copy_scopes boolean default false
) returns jsonb
language plpgsql
security definer
as $$
...
select 'CORRECCIONES APLICADAS CORRECTAMENTE' as status,
  (select count(*) from pg_proc where proname in ('clone_role', 'copy_role_permissions')) as functions_created;
