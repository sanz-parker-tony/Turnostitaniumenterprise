...
-- ✅ CORREGIDO: r.id en lugar de r.role_id
-- =====================================================
SELECT 
    r.role_name,
    s.screen_name,
    ...
    rsa.is_active as rsa_activo
FROM public.roles r
CROSS JOIN public.screens s
CROSS JOIN public.actions a
LEFT JOIN public.screen_actions sa ON sa.screen_id = s.id AND sa.action_id = a.id
LEFT JOIN public.role_screen_actions rsa ON rsa.role_id = r.id AND rsa.screen_action_id = sa.id
WHERE r.role_key = 'SUPER_ADMIN'
...
-- =====================================================
-- PASO 9: NUEVO - Verificar qué pantallas de ORGANIZACIÓN existen
-- =====================================================
SELECT 
    s.screen_key,
    s.screen_name,
...
-- =====================================================
-- PASO 10: NUEVO - Verificar el rol "Administrador Total"
-- (Vi que el usuario tiene DOS roles asignados)
-- =====================================================
SELECT 
    r.role_key,
    r.role_name,
...
FROM public.roles r
LEFT JOIN public.role_screen_actions rsa ON rsa.role_id = r.id
WHERE r.role_name = 'Administrador Total'
